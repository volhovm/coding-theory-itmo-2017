...
data TerminalType = Aqua    -- ^ Output on Mac OS X (Aqua Terminal).
                  ...
                  | Labels String String  -- ^ Axis labels
                  | MainTitle String  -- ^ Main plot title
                  | Grid
...
instance (Fractional x, Enum x, Show x, Num y, Show y) => Plot [Graph2D x y] where
    plot' options term graphs =
        exec options (map toString term) "plot" options' datasources
...
--            ord' (Title _) (Color _) = LT
--            ord' (Labels _) (Color _) = LT
...
instance GnuplotIdiom (TerminalType) where
    toString t = case t of
        ...
        Labels xl yl -> "set xlabel \"" ++ xl ++ "\" font \",10\"; " ++
                        "set ylabel \"" ++ yl ++ "\" font \",10\"; " ++
                        "show xlabel; show ylabel"
        MainTitle t' -> "set title \"" ++ t' ++ "\"; show title"
        Grid -> "set grid ytics lc rgb \"#bbbbbb\" lw 1 lt 0; " ++
